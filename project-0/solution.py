...
def log_posterior_probs(x):
    ...
    # H_i := event that data was sampled from p_i
    # p_i := one of the three distributions
    # X = [x_1, ..., x_k] := the data
    # P(H_i | X) = P(X | H_i) * P(H_i) / P(X)

    probs = []
    for i, dist in enumerate(HYPOTHESIS_SPACE):
        x_given_h = np.sum(dist.logpdf(x))

        h_given_x_unnormalized = x_given_h + np.log(PRIOR_PROBS[i])

        probs.append(h_given_x_unnormalized)

    max_log = np.max(probs)
    log_sum = max_log + np.log(np.sum(np.exp(probs - max_log)))

    log_posteriors_normalized = probs - log_sum

    log_p = np.array(log_posteriors_normalized)
    print(log_p)
